...
export const createInvoiceAmt = async (amt: number) => {
  return await httpPost(`invoice/${amt}`);
}

export const verifyPayment = async (hash: string) => {
  const request = { hash };
  return await httpPost(`verify/payment`, request);
};
